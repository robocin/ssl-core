...
ZmqPublisher::ZmqPublisher(int n_threads)
    : context_(n_threads), publisher_(context_, kSocketType) {}

ZmqPublisher::ZmqPublisher() : ZmqPublisher(kSingleThread) {}

void ZmqPublisher::bind(std::string_view address) {
  publisher_.bind(address.data());
}
...
ZmqSubscriber::ZmqSubscriber(std::string_view topic, int n_threads)
    : ITopicSubscriber(topic), context_(n_threads),
      subscriber_(context_, kSocketType) {}

ZmqSubscriber::ZmqSubscriber(std::string_view topic)
    : ZmqSubscriber(topic, kSingleThread) {}
...
bool ZmqSubscriber::receive(PubSubMode mode, std::string &result) {
  ZmqRecvFlags flags = mode == PubSubMode::DontWait ? ZmqRecvFlags::dontwait
                                                    : ZmqRecvFlags::none;
