...
#include "vision/common/concepts.h"

namespace vision {

enum class PubSubMode : std::uint8_t { Wait = 0, DontWait = 1 };
...
public:
  ...
  virtual void send(std::string_view topic, PubSubMode mode,
  ...
  auto callable_send(std::string_view topic,
                     PubSubMode mode) { // NOLINT(readability-*)
    return [this, topic, mode](auto message) { send(topic, mode, message); };
  }
};
...
public:
  ...
  template <protobuf Pb> bool receive(PubSubMode mode, Pb &proto) {
  ...
  auto callable_receive(PubSubMode mode) { // NOLINT(readability-*)
    return [this, mode](auto &result) { return receive(mode, result); };
  }
