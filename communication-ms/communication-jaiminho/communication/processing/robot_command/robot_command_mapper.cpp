...
rc::RobotInfo RobotCommandMapper::fromNavigationAndReferee(const tp::Referee& referee,
                                                           const rc::Navigation& navigation) {
  rc::RobotInfo result;
  ...
  robot_velocity.mutable_velocity()->set_x(
      navigation.output(navigation.output_size() - 1).forward_velocity());
  robot_velocity.mutable_velocity()->set_y(
      navigation.output(navigation.output_size() - 1).left_velocity());
  robot_velocity.set_angular_velocity(
      navigation.output(navigation.output_size() - 1).angular_velocity());
